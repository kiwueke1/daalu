...
def _run(cmd: Sequence[str], *, stdin_text: Optional[str]=None) -> str:
    p = subprocess.run(
        list(cmd),
        input=stdin_text,
        check=True,
...
class Metal3TemplateGenerator:
    def generate(
        ...
        *,
        ...
        if not bmhs:
            raise RuntimeError(f"No BareMetalHosts found in namespace={opts.namespace}")
        ...
        upsert_clusterctl_vars_block(clusterctl_yaml_path, rendered_block_text)
        ...
        def gen(item: str) -> None:
            ...
            out = opts.temp_gen_dir / f"{opts.capm3_version}_{item}_{opts.image_os}.yaml"
