...
def deploy_csi(inventory="inventory/hosts.ini", tags=None, extra_vars=None):
    """
    Deploy CSI using the csi.yml playbook.
    - inventory: path to Ansible inventory file
    - tags: list of tags or sub-tags to run (e.g., ["rbd", "cephfs"])
    - extra_vars: additional Ansible extra vars
    """
    playbook_name = "csi.yml"  # ✅ only the filename

    # Ensure roles are discoverable
    env = os.environ.copy()
    env["ANSIBLE_ROLES_PATH"] = f"{BASE_DIR}/atmosphere/roles"

    print(f"Running CSI from {env['ANSIBLE_ROLES_PATH']}")
    ...
    if rc.rc != 0:
        raise RuntimeError(f"CSI deployment failed: {rc.status}")
    return rc.status
