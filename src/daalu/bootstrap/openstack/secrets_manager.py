...
class SecretsManager:
    ...
    def load(self) -> "SecretsManager":
        ...
        # Flatten nested sections (e.g. openstack_secrets.*) into top-level keys
        # so regex-based password discovery can find them.
        flat: dict[str, Any] = {}
        for k, v in data.items():
            if isinstance(v, dict):
                for sub_k, sub_v in v.items():
                    flat[str(sub_k)] = sub_v
            else:
                flat[str(k)] = v
    ...
    def get(self, key: str, default: Optional[str] = None) -> Optional[str]:
        ...
        # Try without common prefixes (e.g. openstack_helm_endpoints_)
        for prefix in ("openstack_helm_endpoints_", "openstack_secrets_"):
            if key.startswith(prefix):
                short = key[len(prefix):]
                if short in self._raw:
                    return _as_str(self._raw[short])
        return default
