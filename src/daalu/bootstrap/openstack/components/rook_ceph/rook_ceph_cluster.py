...
class RookCephClusterComponent(InfraComponent):
    ...
    def pre_install(self, kubectl):
        ...
        # --- Ceph monitor operations (via SSH) ---
        ...
        rc, out, err = self._ssh.run(
            "cephadm shell -- ceph quorum_status -f json",
            sudo=True,
        )
        if rc != 0:
        ...
        rc, out, err = self._ssh.run(
            "cephadm shell -- ceph auth get client.admin -f json",
            sudo=True,
        )
        if rc != 0:
        ...
        rc, out, err = self._ssh.run(
            "cephadm shell -- ceph auth get mon. -f json",
            sudo=True,
        )
        if rc != 0:
